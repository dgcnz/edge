...
class DINOTransformer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        assert len(multi_level_feats) == len(multi_level_masks), "mismatched input"
        assert len(multi_level_feats) == len(multi_level_pos_embeds), "mismatched input"
        feat_flatten = []
        mask_flatten = []
        lvl_pos_embed_flatten = []
        spatial_shapes: List[Tuple[int, int]] = []

        bs = multi_level_feats[-1].shape[0]
